...
namespace stream {
  ...
  /**
   * @brief Prints the rest of the formated string, after all variables have
   * been
   * used.
   *
   ...
   */
  inline void print(std::ostream& out, std::string& __format) {
    out << __format;
  }
   ...
   *
   * This function takes any number of variables of any type, where the first
   * one
   * must be an ostream, and the second must be a format style string. Then
   * every
   ...
   *
   * Reads characters from istream until there are no more characters to read,
   * or
   * until on of the characters in `__delim` is read. Then reads data from set
   * of
  ...
  T scan_delim(std::istream& in, std::string __delim, bool __width = false,
               ...
               unsigned int num_fmt = estl::stream::Format::NONE) {
    ...
    if (num_fmt == estl::stream::Format::HEX) {
      ss >> std::hex;
    } else if (num_fmt == estl::stream::Format::OCT) {
      ss >> std::oct;
    } else if (num_fmt == estl::stream::Format::FLOAT_HEX) {
   ...
   * This function takes any number of variables of any type, where the first
   * one
   * must of an istream, and the second must be a format style string. Then
   * every
  ...
  void scan(std::istream& in, std::string __format, T& first, Args&... args) {
    ...
    while (__looping_width == true) {
      if (static_cast<int>(__format[i]) >= 48 &&
          static_cast<int>(__format[i]) <= 57) {
        __scan_width =
            (10 * __scan_width) + (static_cast<int>(__format[i]) - 48);
      } else {
        __looping_width = false;
        i--;
      }
    ...
    switch (__format[i]) {
      ...
      case 'd':
        if (std::is_same<T, int>::value) {
          first =
              estl::stream::scan_delim<T>(in, __delim, __width, __scan_width);
        }
        break;
      case 'u':
        if (std::is_same<T, unsigned int>::value) {
      ...
      case 'o':
        if (std::is_same<T, unsigned int>::value) {
          first = estl::stream::scan_delim<T>(
              in, __delim, __width, __scan_width, estl::stream::Format::OCT);
        }
        break;
      case 'x':
        if (std::is_same<T, unsigned int>::value) {
          first = estl::stream::scan_delim<T>(
              in, __delim, __width, __scan_width, estl::stream::Format::HEX);
        }
        break;
      case 'f':
      case 'e':
      ...
      case 'n':
      default:
        first = estl::stream::scan_delim<T>(in, __delim, __width, __scan_width);
        break;
    }
    i++;
    __format.erase(__format.begin(), __format.begin() + i);
