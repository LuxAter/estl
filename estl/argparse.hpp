        ...
        if (opt == ARG_NONE) {
          if (val.Type() == estl::variable::Variable::Types::UNSIGNED_INT ||
            ...
            return ParseUnsignedInt(val);
                     ...
                     val.Type() ==
                         estl::variable::Variable::Types::CHAR_ARRAY) {
            if (help_ == std::string()) {
              SetHelp(val);
      ...
      void SetNArgs(estl::variable::Variable val) {
        if (val.Type() == estl::variable::Variable::UNSIGNED_INT) {
          ...
          if (nargs == "one") {
            ...
            n_args_ = KLEENE_PLUS;
          } else if (nargs.find_first_not_of("0123456789") ==
                     std::string::npos) {
     ...
     private:
      ...
      void ArgumentRequiredError(int flag) const {
        if (flag == 0) {
          ...
          std::cerr << "ERROR: " << flag
                    << " more arguments are required for \"";
        }
      ...
      estl::variable::Variable GetArgument(std::string arg_str) const {
        ...
        if (argument.IsValid() == false) {
          ...
          std::cerr << "ERROR: Argument (" << arg_str
                    << ") not in choices for \"" << GetNamesStr()
      ...
      bool InChoices(estl::variable::Variable argument) const {
        ...
        if (choices_type == estl::variable::Variable::BOOL_VECTOR &&
          ...
          return false;
                       ...
                       estl::variable::Variable::SIGNED_CHAR_VECTOR &&
          ...
          std::vector<signed short int> vec =
              choices_.GetSignedShortIntVector();
          ...
          if (std::find(vec.begin(), vec.end(),
                        argument.GetUnsignedShortInt()) != vec.end()) {
            return true;
          }
          return false;
        } else if (choices_type ==
                       estl::variable::Variable::SIGNED_INT_VECTOR &&
          ...
          if (std::find(vec.begin(), vec.end(),
                        argument.GetUnsignedLongInt()) != vec.end()) {
            return true;
          }
          return false;
        } else if (choices_type ==
          ...
          return false;
        } else if (choices_type == estl::variable::Variable::
                                       UNSIGNED_LONG_LONG_INT_VECTOR &&
                   arg_type ==
                       estl::variable::Variable::UNSIGNED_LONG_LONG_INT) {
          ...
          return false;
                       ...
                       estl::variable::Variable::LONG_DOUBLE_VECTOR &&
          ...
          return false;
                       ...
                       estl::variable::Variable::CHAR_ARRAY_VECTOR &&
