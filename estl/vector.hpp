...
namespace vector {
  ...
  class Vector {
   public:
     ...
     *
     * @throw out_of_range Thows out of range if initializer_list is greater
     * than
     ...
     * checking. If `i` is not withing the range of the container, an exception
     * of
     ...
     *
     * Returns a reverse iterator to the lement following the last element of
     * the
     ...
     *
     * Returnsd a reverse iterator to the element that is located at `[i]` in
     * the
     * container.
     *
     ...
     * @return Iterator to the specified element.
     */
    reverse_iterator relement(size_type i) {
      return reverse_iterator(data_ + i);
    }
    /**
     * @brief Returns reverse iterator to specified element.
     *
     ...
     *
     * Returns the maximum number of elements the container is able to hold do
     * to
     * system or library implementation limitations, i.e.
     * `std::distance(begin(),
     ...
     *
     * For every element in the container, executs `func` with that element as
     * the
     * argument.
     *
    ...
    inline estl::vector::Vector<_Tp, _N>& operator+=(const _T& rhs) {
      *this = *this + rhs;
      return *this;
    }
    /**
  ...
  /**
   * @relates estl::vector::Vector
   * @name Operators
   * @{ */

  /**
   * @brief Stream output operator for estl::vector::Vector.
   *
  ...
  std::ostream& operator<<(std::ostream& out,
                           const estl::vector::Vector<_Tp, _N> vec) {
    out << '<';
  ...
  inline bool operator==(const estl::vector::Vector<_TpA, _NA>& lhs,
                         const estl::vector::Vector<_TpB, _NB>& rhs) {
    if (_NA != _NB) {
      return false;
    } else {
   ...
   * @param rhs Second vector to compare.
   *
   * @return `true` if the contents of `lhs` are lexicographically *greater*
   * than
  ...
  inline estl::vector::Vector<_Tp, _N> operator+(
      const estl::vector::Vector<_Tp, _N>& lhs, const _T& rhs) {
    estl::vector::Vector<_Tp, _N> vec(lhs);
   ...
   */
  template <typename _TpA, typename _TpB, std::size_t _NA, std::size_t _NB>
  inline estl::vector::Vector<_TpA, _NA> operator+(
      const estl::vector::Vector<_TpA, _NA>& lhs,
      const estl::vector::Vector<_TpB, _NB>& rhs) {
    estl::vector::Vector<_TpA, _NA> vec(lhs);
    typename estl::vector::Vector<_TpA, _NA>::iterator it;
    typename estl::vector::Vector<_TpB, _NB>::const_iterator rhs_it;
   ...
   * @brief Preforms vector arithmetics on a vector and a scalar.
   *
   * Implements the binary operators for vector arithmetic. Divides element of
   * the
   ...
   */
  template <typename _Tp, std::size_t _N>
  _Tp sum(const estl::vector::Vector<_Tp, _N>& lhs) {
    _Tp total = _Tp();
    for (typename estl::vector::Vector<_Tp, _N>::const_iterator it =
             lhs.begin();
         it != lhs.end(); ++it) {
      total += *it;
    }
  ...
  _TpA dot(const estl::vector::Vector<_TpA, _N>& lhs,
           const estl::vector::Vector<_TpB, _N>& rhs) {
    estl::vector::Vector<_TpA, _N> vec = lhs * rhs;
    return sum(vec);
  }
   ...
   */
  template <typename _Tp, std::size_t _N>
  estl::vector::Vector<_Tp, _N> cross(
      const estl::vector::Vector<_Tp, _N>& lhs,
      const estl::vector::Vector<_Tp, _N>& rhs) {
   ...
   *
   * Determins the vector perpendicular to all provided vectors. One must
   * provide
  ...
  estl::vector::Vector<_Tp, _N> cross(estl::vector::Vector<_Tp, _N> vec,
                                      _Vecs... args) {
    std::vector<estl::vector::Vector<_Tp, _N>> vecs =
        estl::variadic::unpack_vector(args...);
    vecs.insert(vecs.begin(), vec);
    estl::vector::Vector<_Tp, _N> res;
    for (std::size_t i = 0; i < res.size(); i++) {
      estl::matrix::Matrix<_Tp, _N - 1, _N - 1> mat;
      ...
      res[i] = estl::matrix::determinant(mat);
    }
    return res;
  }
   ...
   * length will not be accurate, as it requires square root, instead use `_Tp`
   * as
   * a `double` or `float`.
   *
   * @tparam _Tp Value type of the vector.
   ...
   *
   * Returns the normalized version of the provided vector. That is the length
   * of
   ...
   */
  template <typename _Tp, std::size_t _N>
  estl::vector::Vector<_Tp, _N> normalize(estl::vector::Vector<_Tp, _N> vec) {
    estl::vector::Vector<_Tp, _N> new_vec(vec);
    _Tp len = length(vec);
    for (typename estl::vector::Vector<_Tp, _N>::iterator it = new_vec.begin();
         it != new_vec.end(); it++) {
      *it /= len;
    }
    return new_vec;
   ...
   *
   * Converts the vector position into spherical coordinates, such that the
   * first
   ...
   */
  template <typename _Tp, std::size_t _N>
  estl::vector::Vector<_Tp, _N> SphericalCoordinates(
      estl::vector::Vector<_Tp, _N> vec) {
    estl::vector::Vector<_Tp, _N> res;
    *res.begin() = estl::vector::length(vec);
    if (vec.size() == 3) {
      res[1] = acos(vec[2] / res[0]);
      res[2] = atan(vec[1] / vec[0]);
    } else {
      for (typename estl::vector::Vector<_Tp, _N>::iterator
               it = vec.begin(),
               res_it = res.begin() + 1;
           it != vec.end() - 1 && res_it != res.end(); ++it, ++res_it) {
        _Tp len(0);
        for (typename estl::vector::Vector<_Tp, _N>::iterator sub = it;
             sub != vec.end(); ++sub) {
          len += pow(*sub, 2.0);
        }
        len = pow(len, 0.5);
   ...
   *
   * Converts the vector representation into cartesian coordinates, such that
   * the
   ...
   */
  template <typename _Tp, std::size_t _N>
  estl::vector::Vector<_Tp, _N> CartesianCoordinates(
      estl::vector::Vector<_Tp, _N> vec) {
    estl::vector::Vector<_Tp, _N> res;
    if (vec.size() == 3) {
