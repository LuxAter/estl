...
namespace matrix {
   ...
   *
   * `estl::matrix::Matrix` is a container that encapsulates fixed size
   * matrices.
   *
   ...
   * array, it doesn't decay to `T*` automatically. Elements of this container
   * can
  ...
  class Matrix {
   public:
     ...
     *
     * Returns a reference to the element at specified location `__n`, with
     * bounds
    ...
    constexpr const_reference front() const {
      return const_reference(__data[0]);
    }
    /**
     ...
     * @brief Direct access to the underlying array.
     *
     * Returns a `std::array` containing a copy of the underlying array serving
     * as
     * element storage.
     *
     ...
     * `std::array<std::array>` will not effect the values in the matrix.
     *
     * @return `std::vector` containing a copy of the underlying element
     * storage.
     */
     ...
     * element of the non-reversed container. This element acts as a
     * placeholder,
    ...
    inline void operator()(_Tp (*func)(_Tp)) {
      for (typename estl::matrix::Matrix<_Tp, _Nr, _Nc>::iterator it = begin();
           it != end(); ++it) {
        *it = func(*it);
      }
    }
    ...
    inline void swap_row(size_type __a, size_type __b) {
      for (typename estl::matrix::Matrix<_Tp, _Nr, _Nc>::iterator
               it_a = element(__a, 0),
               it_b = element(__b, 0);
     ...
     * column. Does not cause iterators and references to associate with the
     * other
     * column.
     *
    ...
    inline estl::matrix::Matrix<_Tp, _Nr, _Nc>& operator+=(const _T& rhs) {
      *this = *this + rhs;
      return *this;
    }
    /**
  ...
  /**
   * @relates estl::matrix::Matrix
   * @name Operators
   * @{
   */

  /**
   * @brief Stream output operator for estl::matrix::Matrix.
   *
  ...
  std::ostream& operator<<(std::ostream& __out,
                           const estl::matrix::Matrix<_Tp, _Nr, _Nc>& mat) {
    __out << '[';
  ...
  inline bool operator==(const estl::matrix::Matrix<_TpA, _Nr, _Nc>& lhs,
  ...
  inline estl::matrix::Matrix<_A, _R, _C> operator+(
      const estl::matrix::Matrix<_A, _R, _C>& lhs, const _B& rhs) {
    estl::matrix::Matrix<_A, _R, _C> mat;
    typename estl::matrix::Matrix<_A, _R, _C>::iterator it;
    typename estl::matrix::Matrix<_A, _R, _C>::const_iterator lhs_it;
   ...
   * @brief Preforms matrix arithmetics on a matrix and a scalar.
   *
   * Implements the binary operators for matrix arithmetic. Multiplies the
   * scalar
   ...
   * @brief Preforms matrix arithmetics on a matrix and a scalar.
   *
   * Implements the binary operators for matrix arithmetic. Divides every
   * element
   ...
   * be represented by a `_Tp` value. This means that if `_Tp` is `int`, then
   * the
   ...
   */
  template <typename _Tp, std::size_t _Nr, std::size_t _Nc>
  _Tp determinant(const estl::matrix::Matrix<_Tp, _Nr, _Nc>& lhs) {
    estl::matrix::Matrix<_Tp, _Nr, _Nc> mat(lhs);
    _Tp sign = _Tp(1);
    for (typename estl::matrix::Matrix<_Tp, _Nr, _Nc>::size_type it = 0;
         it != mat.rows() && it != mat.columns(); it++) {
      typename estl::matrix::Matrix<_Tp, _Nr, _Nc>::size_type sw;
      ...
      for (typename estl::matrix::Matrix<_Tp, _Nr, _Nc>::size_type row = it + 1;
           row != mat.rows(); row++) {
        _Tp mult = mat.at(row, it) / mat.at(it, it);
        for (typename estl::matrix::Matrix<_Tp, _Nr, _Nc>::size_type column =
                 it;
   ...
   */
  template <typename _Tp, std::size_t _Nr, std::size_t _Nc>
  estl::matrix::Matrix<_Tp, _Nr, _Nc> inverse(
      const estl::matrix::Matrix<_Tp, _Nr, _Nc>& lhs) {
    estl::matrix::Matrix<_Tp, _Nr, _Nc> mat(lhs);
    estl::matrix::Matrix<_Tp, _Nr, _Nc> inv;
   ...
   */
  template <typename _Tp, std::size_t _Nr, std::size_t _Nc>
  estl::matrix::Matrix<_Tp, _Nr, _Nc> echelon_form(
      const estl::matrix::Matrix<_Tp, _Nr, _Nc>& lhs) {
    estl::matrix::Matrix<_Tp, _Nr, _Nc> mat(lhs);
    for (typename estl::matrix::Matrix<_Tp, _Nr, _Nc>::size_type it = 0;
         it != mat.rows() && it != mat.columns(); it++) {
      _Tp pivot = mat.at(it, it);
      for (typename estl::matrix::Matrix<_Tp, _Nr, _Nc>::size_type col = 0;
           col != mat.columns(); col++) {
        mat.at(it, col) /= pivot;
      }
      for (typename estl::matrix::Matrix<_Tp, _Nr, _Nc>::size_type row = 0;
           row != mat.rows(); row++) {
        _Tp mult = mat.at(row, it);
        for (typename estl::matrix::Matrix<_Tp, _Nr, _Nc>::size_type col = it;
