...
class MatrixTest : public testing::Test {
 protected:
  ...
  estl::matrix::Matrix<double, 3, 3> a, b, c;
  estl::matrix::Matrix<double, 0, 0> d;
};

template <typename _Tp, std::size_t _Nr, std::size_t _Nc>
::testing::AssertionResult MatrixMatch(
    estl::matrix::Matrix<_Tp, _Nr, _Nc> lhs,
    estl::matrix::Matrix<_Tp, _Nr, _Nc> rhs) {
...
TEST_F(MatrixTest, Constructor) {
  estl::matrix::Matrix<double, 3, 3> mat(
...
TEST_F(MatrixTest, Functions) {
  b[0] = 5.0;
  EXPECT_EQ(estl::matrix::trace(b), 17.0);
  EXPECT_FLOAT_EQ(estl::matrix::determinant(b), -15.0);
  estl::matrix::Matrix<double, 3, 3> mat = {0.2,  -0.4,         0.2,
                                            -0.4, -28.0 / 15.0, 19.0 / 15.0,
                                            0.2,  29.0 / 15.0,  -17.0 / 15.0};
  EXPECT_TRUE(MatrixMatch(estl::matrix::inverse(b), mat));
  a = {1, 2, 3, 4, 5, 6, 7, 8, 9};
  b = {9, 8, 7, 6, 5, 4, 3, 2, 1};
  mat = {30, 24, 18, 84, 69, 54, 138, 114, 90};
  EXPECT_EQ(estl::matrix::multiplication(a, b), mat);
  estl::matrix::Matrix<double, 3, 4> c = {
      {1, -3, 3, -4}, {2, 3, -1, 15}, {4, -3, -1, 19}};
  estl::matrix::Matrix<double, 3, 4> d = {
      {1, 0, 0, 5}, {0, 1, 0, 1}, {-0, -0, 1, -2}};
  EXPECT_TRUE(MatrixMatch(estl::matrix::echelon_form(c), d));
}
